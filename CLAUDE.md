...
5. **sync_assets** — 同步 skills、output-styles、CLAUDE.md 到 `~/.claude/`（复用 managed_tree + managed_file）
...
`claude_assets/` 下的资源由 playbook 自动发现，不存在则跳过。可通过变量 `claude_code_skills_src`、`claude_code_output_styles_src`、`claude_code_claude_md_src` 显式覆盖路径。
